...
|---|---|---|
> ⚠ **最新の正準的な数値は README.md の「汎化性能」「棄却について」に一本化**した。本節は設計意図の
> 記録であり、ここに点在していた具体値（77.4%/71.4%/0.857/「out 6/6 誤受理0」等）は陳腐化していた
> ため撤去した。現況は `python validate_real.py` / `python reject_eval.py` で随時確認すること。

- 自己検索でタグを含めると Hit@1=100% になるが、これは **DB 内飽和であって汎化ではない**
  （特徴量と beta は同じコーパスで選んでいるため同語反復）。実機クエリはタグを持たないため、
  汎化はトポロジーのみで報告する（下記 ablation 参照）。

### 10.2 棄却＝OOD 検知（§7 への回答）

旧 LOO θ（§3 Step4 の 0.9786）は「素のDB」の自己類似(≈1.0)基準で、実機を全棄却し実用不能。
