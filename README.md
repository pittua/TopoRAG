...
├── sample_netlists.json     DB登録済み回路のネットリスト定義（31回路）
...
| 要素 | 計算方法 | 説明 |
|---|---|---|
| `topo_score` | コサイン類似度（38次元ベクトル）または階層マッチング | 回路の構造・接続トポロジーの一致度 |
...
**ablation（特徴グループの寄与）**：各グループをゼロ化し Hit@1 低下で寄与を測ります（alpha=1.0）。
トポロジーのみのベースラインは自己検索 77.4% / 実機 71.4%。能動次元(19-33)を除くと**実機 Hit@1 が
−21.4pt** と最も影響が大きく、ダイオード役割次元(34-37)も実機 −7.1pt 寄与します（旧・向き次元 9-12 は
実機 ±0）。タグは自己検索を +22.6pt 押し上げますが**実機クエリはタグを持たないため汎化に寄与しません**
（自己検索 100% はタグ依存）。このため汎化性能はトポロジーのみで報告します。

- `real_corpus.json` … 変換済み実機回路、`real_expected.yaml` … 各回路の正解ラベル（in/out-of-scope）。
- **棄却はトポロジーのみ(alpha=1.0)の top-1 スコアで判定**します（タグ非依存）。実機20回路の
  校正で top-1 絶対スコアが最も分離性能が高く（AUC 0.857）、margin/ratio はほぼ無力でした。
- 実装は `CircuitRAG.search_with_rejection()`。閾値 `RECOMMENDED_REJECT_THRESHOLD`（既定 0.83、
  `reject_eval.py` で校正）未満なら空リスト（識別不能）を返します。out-of-scope 6/6 を誤受理 0 で棄却。
- **スイッチング素子の正規化**：DB はコンバータのスイッチを `SW` でモデル化しますが、実機は実
  ...
  一致します（増幅用 MOSFET は不変）。実機 in-scope の Hit@1 64%→71%、Hit@3 →100%、MRR →0.845。
