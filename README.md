# TopoRAG — 回路ネットリストの構造認識（知覚層IR + 構造検索 + 知識カード + LLM 認識器）

ネットリスト（JSON形式の回路接続情報）を入力として、
**手設計の 43 次元グラフ特徴ベクトルによる最近傍検索**で近縁回路を引き、
決定的な知覚層が出力する«構造IR»と、近縁候補に付く«弁別カード»（識別の決め手）を
**LLM 認識器が照合して回路種別を裁定する**システムです。
複合・多段回路はブロック単位の照合で対応します。

> **プロジェクトの目的（2026-06 再定義）**：最終成果物は回路分類器ではなく、
> **LLM に回路認識能力を与えること**。LLM はグラフ構造を直接追えないため、決定的な知覚層
> （`feature_extractor` / `block_decomposer` → `circuit_ir`）が消化済みの構造事実に翻訳し、
> 紛らわしい近縁の弁別知識を«知識カード»としてコーパスに載せる。LLM 段はネットリストを推論せず、
> IR の構造事実とカードの決め手を一つずつ照合して裁定する «認識器» として働く
> （仕様は `docs/IR_SPEC.md` / `docs/CARD_SPEC.md`）。

> **正直な位置づけ**：検索段は学習済み埋め込みやスケール戦略を持つ「RAG」ではなく、
> **47 個の手書きプロトタイプに対する手設計特徴の最近傍分類器**です（名前に "RAG" を含むのは
> 経緯的なもの）。汎化性能は実機 in=28 の検証（CI は依然広い）でのみ測れ、自己検索 Hit@1=100% は
> 汎化ではありません（下記「汎化性能」参照）。LLM 認識器段は実験A（①LLM 直読み）・実験B（③IR+カード裁定）で
> 評価済みで、指標も「正解名の文字列一致率」から family 単位の strict/lenient Hit@1＋Wilson 95%CI
> （`real_expected.yaml` 準拠）に置換しました（結果: `results/RESULT_experiment_A_2026-07-05.md` /
> `results/AUDIT_experiment_B_2026-07-09.md`）。**ただし結果は率直に芳しくありません**：θ ゲートを揃えた
> 21 件比較で 検索のみ② 85.7% vs IR+カード裁定③ 81.0%（差 −4.7pt・n=21 で有意でない、正味 回復2−破壊3=−1件）と、
> カード裁定は検索を上回りませんでした。当初計画した「+IR のみ（カードなし）」の中間アームは未実行で、
> 純粋な 3アーム ablation は未完です（次の一手）。
...
circuit_rag.py
  ...
  │
  ├─ 単一ブロック回路 ──→ 認識器プロンプト
  │     クエリ = ラベル抑制«構造IR»（circuit_ir.render_ir, reveal_labels=False）
  │     候補   = 弁別カード（あれば）／無ければラベル抑制IR
  │     → LLM に「IR の構造事実とカードの決め手を照合し、該当候補 or『該当なし』を裁定せよ」
  │
  └─ 複合回路 ──────────→ ブロック単位RAGプロンプト（+全体IR）
        → LLM に「ブロック照合結果を読んでブロック名を組み合わせよ」
  │
  ▼
...
`build_prompt()` は回路の構造に応じて 2 つのプロンプトを使い分けます。

```
単一ブロック（認識器）:
  回路全体 → DB検索（上位K件）
  クエリ = ラベル抑制«構造IR»（タグ・説明を伏せてカンニング防止）
  候補   = 弁別カード（あれば決め手・近縁との差分）／無ければラベル抑制IR
  → LLMに「IR の構造事実とカードの決め手を照合し、該当候補 or『該当なし』を裁定せよ」
  → 出力: 【判定】【根拠】【近縁との区別】
...
複合回路（ブロック単位 RAG）:
  ...
  （+ 回路全体のラベル抑制IRも添付）
  → LLMへの指示: 「照合結果を読んでブロック名を組み合わせよ。トポロジー推論は不要」
  → 出力: 【判定】【ブロック構成】【根拠】
```

> **ラベル抑制（reveal_labels=False）について**：機能タグ・説明は実質的に正解ラベルなので、
> 判定対象クエリ側の IR では伏せる（`circuit_ir.render_ir(reveal_labels=False)`）。検索でヒットした
> 候補回路の名称提示は RAG の正常動作として残す。これにより LLM 段の「カンニング」を防ぐ。

単一ブロックでは LLM が IR とカードを照合して裁定し、複合回路では「ブロック名の結合」のみを行う
（いずれもネットリストからのトポロジー推論は発生しない——構造は IR が消化済み）。
...
### 4. 検索段は「RAG」ではなく最近傍分類器／認識器層は検索を上回っていない
検索段の実体は 47 個の手書きプロトタイプに対する手設計 43 次元ベクトルの最近傍分類器であり、
学習済み埋め込みもスケール戦略も持たない。LLM 認識器段は実験A（①LLM 直読み）・実験B（③IR+カード裁定）で
評価済みで、指標も「正解名の文字列一致率」から family 単位 strict/lenient Hit@1＋Wilson 95%CI
（`real_expected.yaml`）へ置換した。**結果はカード裁定が検索を上回らないというもの**で、θ ゲートを揃えた
21 件では 検索のみ② 85.7% vs 裁定③ 81.0%（差 −4.7pt・有意でない、正味 回復2−破壊3=−1件）
（`results/AUDIT_experiment_B_2026-07-09.md`）。実験A の①LLM 直読みは in strict 78.6% で②に劣位だが、
out 棄却は 95.0% と②の 90.0% を上回る（逆転、`results/RESULT_experiment_A_2026-07-05.md`）。知識カードは
16 枚まで執筆したが 47 候補の全被覆には至らず、当初計画の「+IR のみ（カードなし）」中間アームも未実行で、
純粋な 3アーム ablation は未完。
