...
> だけで、その評価指標は「正解名の文字列一致率」という脆いものです。汎化性能は実機 n=15 の
> 検証（CI が広い）でのみ測れ、自己検索 Hit@1=100% は汎化ではありません（下記「汎化性能」参照）。
...
### 汎化性能（実機 n=15・唯一の汎化指標）

第三者が描いた実機回路（タグ無し）で測る。`validate_real.py` の現行値：

| 集計 | Hit@1 | Hit@3 | MRR |
|---|---|---|---|
| strict（厳密一致） | **80.0%** [95%CI 54.8–93.0] | 80.0% | 0.828 |
| lenient（近縁許容） | **86.7%** [95%CI 62.1–96.3] | 93.3% [70.2–98.8] | 0.917 |
...
> 同語反復であり**汎化の証拠ではない**。汎化を示すのは上表（実機 n=15）のみで、**n=15 では CI が広く、
> 64%/71%/80% といった差は全てノイズ幅の中**にある。

**ablation（`ablation.py`, alpha=1.0 / beta=1.0 コサインのみ）**：ベースラインは自己検索 76.6% /
実機 86.7%。能動次元(19-33)を除くと**実機 Hit@1 が −20.0pt** と最も影響が大きい。タグは自己検索を
+23.4pt 押し上げますが**実機クエリはタグを持たないため汎化には寄与しません**（自己検索の底上げは
タグ依存）。

### 棄却について（⚠ 既定閾値は未校正・製品判定に使わないこと）

`real_corpus.json` の out-of-scope（未知）回路は **現状 5 件のみで統計的に無情報**。単一閾値では
in/out をクリーンに分離できない：

- **分離余裕 = −0.20**（負＝重なりあり）。out 最高 `generic_opamp_bip` = **0.8725** が in 最低正解
  `lc-series-resonance` = 0.8297 を**上回る**。
- top-1 スコアの棄却 AUC = 0.867（95%CI [0.653, 1.000]）だが、**nested CV balanced acc = 0.600**
  ＝事前登録基準 0.65 未満（楽観バイアス +0.233）。
- `reject_eval.py` の撤退ゲート判定 = **INSUFFICIENT**（n_out=5 < 事前登録 20）。

実装は `CircuitRAG.search_with_rejection()`、閾値 `PROVISIONAL_REJECT_THRESHOLD`（既定 0.83）。
**この既定 0.83 では `generic_opamp_bip`(out, 0.8725) を誤受理し、`lc-series`(in, 0.8297) を誤棄却する。**
製品の受理/棄却判定として信頼してはならない。`docs/HANDOFF_2026-06-09.md` の事前登録ルールに従い、
out を 20 件以上に拡充して再評価するまで「未校正」とし、拡充後もなお分離不能ならランカー
（top-3 ＋スコア提示・受理判断は人）へ製品定義を切り替える。
...
汎化の証拠ではない。汎化を測れるのは実機 n=15 のみで、その CI は広い（上記「汎化性能」）。

### 3. 小標本ゆえ改善か偶然か判別できない
実機 in=15 / out=5 では比率の 95%CI が ±0.2〜0.3 に達する。64%/71%/80% といった「改善」は全てノイズ幅の
中にあり、有意差として主張できない。棄却は **未校正**（撤退ゲート判定 INSUFFICIENT、`PROVISIONAL_REJECT_THRESHOLD`）。
